...
function run() {
	...
	const isInSubfolder = pathToSearch !== vaultPath; // Use the original logic
	...
	for (const absolutePath of folderArray) {
	    ...
	    // Construct the relative path to the README file
	    const readmeRelativePath = `${relativePath}/README.md`;

	    // Log the name variable
    	console.log(`Name: ${name}`);

	    if (!name) continue; // skip root folder

	    // Use the relative path to form the 'arg' property
	    resultsArr.push({
	        title: name,
	        match: alfredMatcher(name) + " repository ",
	        subtitle: "▸ Open README.md",
	        arg: readmeRelativePath, // Use the relative path for the README file
	        type: "file:skipcheck",
	        ...
	        mods: {
			  ...
			  shift: {
			    valid: true,
			    subtitle: 'Open Keynote',
			    arg: `${relativePath}/${name}.key` 
			  },
			  alt: {
			    valid: true, 
